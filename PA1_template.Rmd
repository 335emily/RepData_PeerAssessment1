...
```
The mean number of steps is: `r stepsByDayMean`
The median number of steps per day is: `r stepsByDayMedian`
...
```
The 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps is the interval beginning at `r maxIntervalTime`
...
```
The numer of missing values is `r numMissingCounts`
...
Specifically, before removing the NA values, the mean was `r stepsByDayMean` and now it is `r stepsByDayMean2`
For the median, before removing the NA values, the median was `r stepsByDayMedian` and now it is `r stepsByDayMedian2`
